				...
				<form action="" method="post">
					<div class="form-group row">
						<div class="col-12 col-sm-12">
							<input type="text" class="form-control" id="firstname" name="admin_username" placeholder="Enter your name" required>
						</div>
					</div>
					<div class="form-group row">
						<div class="col-12 col-sm-12">
...
{	
	$usrname = $_POST['admin_username'];
	$password = $_POST['admin_password'];
	
	$securepass = hash('sha512', $password);

	$query = "SELECT * FROM `admin` WHERE `username` = '$usrname' AND `password` = '$securepass'";
	...
	if($check == true)
	{
		...
		if($name)
		{
			...
			$_SESSION['admin_username'] = $usrname;
			header('location:admin_dashboard.php');
		}
		else
		{
