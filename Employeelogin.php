				...
				<form action="" method="post">
					<div class="form-group row">
						<div class="col-12 col-sm-12">
							<input type="number" class="form-control" id="id" name="emp_id" placeholder="Enter your ID" required>
						</div>
					</div>
					<div class="form-group row">
						<div class="col-12 col-sm-12">
							<input type="text" class="form-control" id="firstname" name="emp_username" placeholder="Enter your name" required>
						</div>
					</div>
					<div class="form-group row">
						<div class="col-12 col-sm-12">
							<input type="password" class="form-control" id="password" name="emp_password" placeholder="Enter your password" required>
						</div>
					</div>
					<div class="form-group row">
						<div class="col-12 col-sm-12 text-center form-btn">
							<input type="submit" name="emp_submit" class="btn btn-light">
						</div>
					</div>
				</form>
			</div>
...
{	
	$id = (int)$_POST['emp_id'];
	$usrname = $_POST['emp_username'];
	$password = $_POST['emp_password'];
	
	$securepass = hash('sha512', $password);

	$query = "SELECT * FROM `retailemployee` WHERE `id` = '$id' AND `username` = '$usrname' AND `password` = '$securepass'";
	...
	if($check == true)
	{
		...
		if($name)
		{
			...
			$_SESSION['emp_name'] = $_POST['emp_username'];
			$_SESSION['emp_submit'] = $_POST['emp_submit'];
			header('location:emp_dashboard.php');
		}
		else
		{
